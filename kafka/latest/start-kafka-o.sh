...
if [[ -z "$KAFKA_LOG_DIRS" ]]; then    # string is  null, has zero length.
...
if [[ -n "$HOSTNAME_COMMAND" ]]; then   # string is not null.
    HOSTNAME_COMMAND=`hostname` # ip route show 0.0.0.0/0 dev eth0 | cut -d\  -f3
    echo "HOSTNAME_COMMAND is present: $HOSTNAME_COMMAND"
    HOSTNAME_VALUE="$HOSTNAME_COMMAND" # HOSTNAME_VALUE=$(eval "$HOSTNAME_COMMAND")
     echo "HOSTNAME_VALUE : $HOSTNAME_VALUE"
    # Replace any occurences of _{HOSTNAME_COMMAND} with the value
    IFS=$'\n'
    for VAR in $(env); do
        if [[ $VAR =~ ^KAFKA_ && "$VAR" =~ "_{HOSTNAME_COMMAND}" ]]; then
            echo "replacing with hostname value..."
...
echo "#####################################################################"
           ...
           while read line
            do
            echo "$line"
           done < "$KAFKA_HOME/config/server.properties"

#exec "$KAFKA_HOME/bin/kafka-server-start.sh"  "$KAFKA_HOME/config/server.properties"
#run in background
nohup $KAFKA_HOME/bin/kafka-server-start.sh $KAFKA_HOME/config/server.properties > /dev/null 2>&1 &
#exec "$KAFKA_HOME/bin/kafka-server-start.sh" -daemon  "$KAFKA_HOME/config/server.properties" &