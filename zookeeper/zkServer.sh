...
if [ -z "$ZOOPIDFILE" ]; then   # true if length of string is zero
...
start)
    ...
    echo  -n " zoopidfile : $ZOOPIDFILE "
    if [ -f "$ZOOPIDFILE" ]; then
        echo  -n "ZOOPIDFILE  exists: $ZOOPIDFILE"
    ...
    if [ $? -eq 0 ]
    then
      if /bin/echo -n $! > "$ZOOPIDFILE"   # $! is the PID of the last program your shell ran in the background
      then
        sleep 1
        pid=$(cat "${ZOOPIDFILE}")
         echo  -n "pid of zookeeper: $pid "
