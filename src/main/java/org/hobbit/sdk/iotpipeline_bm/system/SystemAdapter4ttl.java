...
public class SystemAdapter4ttl extends AbstractSystemAdapter {
	private static final Logger logger = LoggerFactory.getLogger(SystemAdapter4ttl.class);
	...
	* for carbon-graphite
	*/
	private String carbonGraphiteContainerID = null;
...
/*
		// check cluster size
		iterator = systemParamModel.listObjectsOfProperty(systemParamModel
				.getProperty(SYSTEM_PARAM_URI_PREFIX + PIPELINE_SIZE));
		int pipeline_size = Integer.parseInt(iterator.next().asLiteral().getString());*/

		iterator = systemParamModel.listObjectsOfProperty(systemParamModel
				//.getProperty(SYSTEM_PARAM_URI_PREFIX + ZOOKEEPER_INIT_STEP));
				.getProperty("http://project-hobbit.eu/iotpipe-system/zookeeper_init_step"));
		String zookeeper_init_step = iterator.next().asLiteral().getString();

		iterator = systemParamModel.listObjectsOfProperty(systemParamModel
				//.getProperty(SYSTEM_PARAM_URI_PREFIX + KAFKA_INIT_STEP));
				.getProperty("http://project-hobbit.eu/iotpipe-system/kafka_init_step"));
		String kafka_init_step = iterator.next().asLiteral().getString();

		iterator = systemParamModel.listObjectsOfProperty(systemParamModel
				//.getProperty(SYSTEM_PARAM_URI_PREFIX + HBASE_INIT_STEP));
				.getProperty("http://project-hobbit.eu/iotpipe-system/storm_init_step"));
		String hbase_init_step = iterator.next().asLiteral().getString();

		iterator = systemParamModel.listObjectsOfProperty(systemParamModel
				//.getProperty(SYSTEM_PARAM_URI_PREFIX + STORM_INIT_STEP));
				.getProperty("http://project-hobbit.eu/iotpipe-system/hbase_init_step"));
		String storm_init_step = iterator.next().asLiteral().getString();




		// create carbon-graphite container
		setupCarbonGraphite();
	...
	public void receiveGeneratedData(byte[] data) {
		...
		iterator = systemParamModel.listObjectsOfProperty(systemParamModel
			//	.getProperty(SYSTEM_PARAM_URI_PREFIX + DATASET_FOLDER_NAME));
				.getProperty("http://project-hobbit.eu/iotpipe-system/datasetFolderName"));
...
*/
		...
		switch (taskName) {
			...
			// :: read  task  from data ::
				...
				try {
					//FileUtils.writeByteArrayToFile(new File(taskDir + "/" + taskId  + "spike_detection_with_dependencies.jar"), workloadFileInBytes);

					String workloadFileName = taskId + "_" + "spike_detection_with_dependencies.jar"; // TODO  :  remove name shortcut and read from data bytes instead

					FileUtils.writeByteArrayToFile(new File(taskDir + "/" + taskId  + "_spike_detection_with_dependencies.jar"), workloadFileInBytes);
		...
		//		File f = new File(taskDir + "/" + taskName + ".jar");    // in production env, this should be sent over n/w via eg SimpleFilerSender
				File f = new File(taskDir + "/" + taskId  + "_spike_detection_with_dependencies.jar");    // in production env, this should be sent over n/w via eg SimpleFilerSender
...
//----------------------------------------------------------------------------------------------------------------------------------------------
	...
	private void setupZookeeper() throws DockerCertificateException {
		...
		// cluster size:
		iterator = systemParamModel.listObjectsOfProperty(systemParamModel
				//.getProperty(SYSTEM_PARAM_URI_PREFIX + ZOOKEEPER_CLUSTER_SIZE));
				.getProperty("http://project-hobbit.eu/iotpipe-system/zookeeperClusterSize"));
		String zookeeper_cluster_size = iterator.next().asLiteral().getString();

		// zookeeper client port(s):
		iterator = systemParamModel.listObjectsOfProperty(systemParamModel
				//.getProperty(SYSTEM_PARAM_URI_PREFIX + ZOO_CLIENT_PORTS));
				.getProperty("http://project-hobbit.eu/iotpipe-system/zooClientPort"));
		String zoo_client_ports = iterator.next().asLiteral().getString();

		// zookeeper follower port(s):
		iterator = systemParamModel.listObjectsOfProperty(systemParamModel
				//.getProperty(SYSTEM_PARAM_URI_PREFIX + ZOO_FOLLOWER_PORTS));
				.getProperty("http://project-hobbit.eu/iotpipe-system/zooFollowerPorts"));
		String zoo_follower_ports = iterator.next().asLiteral().getString();

		// zookeeper election port(s):
		iterator = systemParamModel.listObjectsOfProperty(systemParamModel
				//.getProperty(SYSTEM_PARAM_URI_PREFIX + ZOO_ELECTION_PORTS));
				.getProperty("http://project-hobbit.eu/iotpipe-system/zooElectionPorts"));
		String zoo_election_ports = iterator.next().asLiteral().getString();


		zookeeperClusterSize = Integer.parseInt(zookeeper_cluster_size);
		...
		logger.debug("--------------------  zk containers created, going to wait for initialization...");
		waitForZookeeperClusterToInit();
		logger.debug("-------------------- zk cluster is ready. ");
		...
		logger.debug("-------------------- zk ensemble config is sent to zk containers for further configuration. ");
		waitForZookeeperClusterToStart();
		logger.debug("-------------------- Zookeeper cluster setup is finished. Zookeeper containers have been started and configured successfully! ");
...
//----------------------------------------------------------------------------------------------------------------------------------------------
	...
	private void  setupCarbonGraphite(){
		...
		if (containerId == null) {
			...
			carbonGraphiteContainerID = containerId;
		}
...
//----------------------------------------------------------------------------------------------------------------------------------------------
	...
	public void waitForCarbonGraphitSetup() {
...
//----------------------------------------------------------------------------------------------------------------------------------------------
	...
	public void setupKafkaCluster() throws IOException {
		...
		// cluster size:
		iterator = systemParamModel.listObjectsOfProperty(systemParamModel
				//.getProperty(SYSTEM_PARAM_URI_PREFIX + KAFKA_CLUSTER_SIZE));
				.getProperty("http://project-hobbit.eu/iotpipe-system/kafkaClusterSize"));
		String kafka_cluster_size = iterator.next().asLiteral().getString();

		kafkaClusterSize = Integer.parseInt(kafka_cluster_size);


		// zookeeper kafka hostname command:
		iterator = systemParamModel.listObjectsOfProperty(systemParamModel
				//.getProperty(SYSTEM_PARAM_URI_PREFIX + KAFKA_HOSTNAME_COMMAND));
				.getProperty("http://project-hobbit.eu/iotpipe-system/kafkaHostnameCommand"));
		...
		iterator = systemParamModel.listObjectsOfProperty(systemParamModel
				//.getProperty(SYSTEM_PARAM_URI_PREFIX + KAFKA_CREATE_TOPICS));
				.getProperty("http://project-hobbit.eu/iotpipe-system/kafkaCreateTopics"));
	...
	private String[] createKafkaContainers(String kafkaImageName, int kafkaClusterSize, String[] envVariables) {
		...
		iterator = systemParamModel.listObjectsOfProperty(systemParamModel
				// 	.getProperty(SYSTEM_PARAM_URI_PREFIX + KAFKA_INSIDE_PORTS));
				.getProperty("http://project-hobbit.eu/iotpipe-system/kafkaInsidePorts"));
		String kafka_inisde_ports = iterator.next().asLiteral().getString();
		String[] kafkaInsidePorts = kafka_inisde_ports.split(",");

		// zookeeper kafka outside port(s):
		iterator = systemParamModel.listObjectsOfProperty(systemParamModel
			//	.getProperty(SYSTEM_PARAM_URI_PREFIX + KAFKA_OUTSIDE_PORTS));
				.getProperty("http://project-hobbit.eu/iotpipe-system/kafkaOutsidePorts"));
...
//----------------------------------------------------------------------------------------------------------------------------------------------
	...
	private void checkKafkaStatus() throws Exception {
		logger.debug(" ----------------Check  Kafka Cluster Status   ----------------");
		...
		logger.debug(" -----------------  Kafka Cluster is {}  ", status);


		/*
		for(int z=0; z<kafkaClusterSize; z++){
			kafkaBrokerURIsArr[z] = kafkaContainerIDs[z] + ":" + kafkaInsidePorts[z]; // null pointer exception?
		}

			KafkaConfiguration config = new KafkaConfiguration();
		config.zookeeperUris = kafkaZookeeperConnect;
		config.brokerUris = kafkaBrokerURIs;
...
//--
	...
	public void setupStormCluster() throws IOException {
		...
		// cluster size:
		iterator = systemParamModel.listObjectsOfProperty(systemParamModel
				.getProperty(SYSTEM_PARAM_URI_PREFIX + NUM_OF_STORM_SUPERVISORS));
		...
		stormEnvVariables = new String[]{
				...
				"GRAPHITE_HOST"        + "=" +  carbonGraphiteContainerID,   // graphite container id to set in storm.yaml for metrics monitoring with carbon-graphite
				"GRAPHITE_PORT"		   + "=" +  "2003"
		};
...
// add nimbus.seeds env variable, and create supervisor containers
		stormEnvVariables = (String[]) ArrayUtils.add(stormEnvVariables, "STORM_NIMBUS_SEEDS=" + String.join(",", stormNimbusContainerIDs));
...
//----------------------------------------------------------------------------------------------------------------------------------------------
	...
	private void printClusterConfigForDoc() {
		...
		stormNimbusContainerIDs = new String[numOfNumbisContainers];  //one container
