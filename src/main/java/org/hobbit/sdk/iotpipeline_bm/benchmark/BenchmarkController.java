...
//###############################################################################################  //
		...
		//iterator = benchmarkParamModel.listObjectsOfProperty(benchmarkParamModel.getProperty(BENCHMARK_PARAM_URI_PREFIX + BENCHMARK_MODE));
		//benchmark_mode += iterator.next().asLiteral().getString();
		benchmark_mode = getPropertyOrDefault(BENCHMARK_PARAM_URI_PREFIX + BENCHMARK_MODE, "pipeline");

		// Sequential tasks
		if (sequentialTasks == false) {
			//	iterator = benchmarkParamModel.listObjectsOfProperty(benchmarkParamModel.getProperty(SEQUENTIAL_TASKS));
			// String st = iterator.next().asLiteral().getString();
			sequentialTasks = getPropertyOrDefault(BENCHMARK_PARAM_URI_PREFIX + SEQUENTIAL_TASKS, false);
		}
		...
		// iterator = benchmarkParamModel.listObjectsOfProperty(benchmarkParamModel.getProperty(BENCHMARK_PARAM_URI_PREFIX + DATA_INPUT_FORMAT));
		//String data_input_format = iterator.next().asLiteral().getString();

		String data_input_format = (String) getPropertyOrDefault(BENCHMARK_PARAM_URI_PREFIX + DATA_INPUT_FORMAT, "dat");

		// in caseof pre-gen data
		//iterator = benchmarkParamModel.listObjectsOfProperty(benchmarkParamModel.getProperty(BENCHMARK_PARAM_URI_PREFIX + PREGEN_DATASETS));
		//String pregenDatasets = iterator.next().asLiteral().getString();
		String pregenDatasets = getPropertyOrDefault(BENCHMARK_PARAM_URI_PREFIX + PREGEN_DATASETS, "sensors.dat");

		// in caseof pre-gen data present remotely
		//iterator = benchmarkParamModel.listObjectsOfProperty(benchmarkParamModel.getProperty(BENCHMARK_PARAM_URI_PREFIX + EXTERNAL_DATA_GEN));
		//	String external_data_gen = iterator.next().asLiteral().getString();

		// loading data scale factor
		//	iterator = benchmarkParamModel.listObjectsOfProperty(benchmarkParamModel.getProperty(BENCHMARK_PARAM_URI_PREFIX + DATA_SIZE_SCALE_FACTOR));
		//	int data_size_scale_factor = Integer.parseInt(iterator.next().asLiteral().getString());
		int data_size_scale_factor = getPropertyOrDefault(BENCHMARK_PARAM_URI_PREFIX + DATA_SIZE_SCALE_FACTOR, 1);


		// num of times each task must be operated
		//	iterator = benchmarkParamModel.listObjectsOfProperty(benchmarkParamModel.getProperty(BENCHMARK_PARAM_URI_PREFIX + NUM_OF_OPS));
		//	int num_of_ops = Integer.parseInt(iterator.next().asLiteral().getString());
		int num_of_ops = getPropertyOrDefault(BENCHMARK_PARAM_URI_PREFIX + NUM_OF_OPS, 1);


		// loading tasks
		//	iterator = benchmarkParamModel.listObjectsOfProperty(benchmarkParamModel.getProperty(BENCHMARK_PARAM_URI_PREFIX + TASKS));
		//	String tasks = iterator.next().asLiteral().getString();
		String tasks = getPropertyOrDefault(BENCHMARK_PARAM_URI_PREFIX + TASKS, "spike_detection");
		...
		tasks = getPropertyOrDefault(BENCHMARK_PARAM_URI_PREFIX + TASKS, "spike_detection");


		// loading seed
		//iterator = benchmarkParamModel.listObjectsOfProperty(benchmarkParamModel.getProperty(BENCHMARK_PARAM_URI_PREFIX + SEED));
		//int seed = Integer.parseInt(iterator.next().asLiteral().getString());
	...
	/*	iterator = benchmarkParamModel.listObjectsOfProperty(benchmarkParamModel
     ...
     /*
        ...
        */
		...
		evalStorage_env_variables = (String[]) ArrayUtils.add(DEFAULT_EVAL_STORAGE_PARAMETERS, "HOBBIT_RECEIVE_TIMESTAMP_FOR_SYSTEM_RESULTS=" + "true");
		evalStorage_env_variables = (String[]) ArrayUtils.add(DEFAULT_EVAL_STORAGE_PARAMETERS, "HOBBIT_EXPERIMENT_URI=" + this.experimentUri);
			...
			logger.info("tasks : non-seqential");
			evalStorage_env_variables = ArrayUtils.add(evalStorage_env_variables, "ACKNOWLEDGEMENT_FLAG=false");
		}

		evalStorage_env_variables = ArrayUtils.add(evalStorage_env_variables, "HOBBIT_RECEIVE_TIMESTAMP_FOR_SYSTEM_RESULTS=true");
		evalStorage_env_variables = ArrayUtils.add(evalStorage_env_variables, "HOBBIT_EXPERIMENT_URI=" + this.experimentUri);
